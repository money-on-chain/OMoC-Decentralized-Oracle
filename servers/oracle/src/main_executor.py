...
class MainExecutor:
    ...
    def __init__(self):
        ...
        self.oracle_loop = OracleLoop(self.oracle_account)
    ...
    def setup(self):
        ...
        if oracle_settings.ORACLE_MONITOR:
            monitor.log_setup()
            self.tasks.append(monitor.MonitorTask())
        if oracle_settings.SCHEDULER_RUN_SUPPORTERS_SCHEDULER:
            self.tasks.append(BgTaskExecutor(scheduler_supporters_loop.scheduler_supporters_loop))
