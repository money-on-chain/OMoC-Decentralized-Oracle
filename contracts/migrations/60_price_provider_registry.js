...
async function deploy(config) {
    ...
    await scripts.add({
        contractsData: [{name: 'PriceProviderRegister', alias: 'PriceProviderRegister'}],
    });
    // Give more gas!!!
    ...
    console.log(
        ...
        priceProviderRegister.options.address,
        'proxyAdmin',
        config.proxyAdminAddr,
    );

    console.log(
        'Register the OracleManager coinpairs in price provider register by gobernanza',
        config.governorAddr,
    );
    const ChangeContract = artifacts.require('PriceProviderOracleManagerRegisterPairChange');
    const change = await ChangeContract.new(
        priceProviderRegister.options.address,
        oracleManagerAddr,
    );
