...
function config() {
    const vars = [
        ...
        'supportersMinStayBlocks',
        'supportersAfterStopBlocks',
    ];

    if (vars.some((x) => !process.env[x])) {
        console.error(
            'The script must be configured with .env file some parameter is missing, see dotenv_example',
        );
        console.error(vars);
        process.exit();
    }
...
function is_test() {
    const ret =
        process.argv.some((x) => x === 'test') || process.argv.some((x) => x === 'coverage');
    if (ret) {
