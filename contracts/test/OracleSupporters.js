...
contract.skip('Oracle-Supporters integration', (accounts) => {
    ...
    it('Stake', async () => {
        ...
        await this.oracleMgr.subscribeToCoinPair(oracle1, web3.utils.asciiToHex('BTCUSD'), {
            from: oracle1,
        });
        ...
        mocs = await this.token.balanceOf(oracle1);
        expect(mocs, 'Oracle1 MOC balance').to.be.bignumber.equal(
            INITIAL_BALANCE.sub(STAKE).sub(STAKE),
        );
    });
    ...
    it('Withdraw', async () => {
        ...
        await this.oracleMgr.subscribeToCoinPair(oracle2, web3.utils.asciiToHex('BTCUSD'), {
            from: oracle2,
        });
        ...
        await this.oracleMgr.unsubscribeFromCoinPair(oracle1, web3.utils.asciiToHex('BTCUSD'), {
            from: oracle1,
        });
