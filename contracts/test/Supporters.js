...
contract('Supporters', (accounts) => {
    ...
    describe('Deposits', () => {
        ...
        it('deposit earnings', async () => {
            ...
            await expectRevert(
                supporters.distribute({from: payer}),
        ...
        it('multiple withdrawals', async () => {
            ...
            await users.reduce((seq, user) => {
                return seq.then(async () => {
                    ...
                    expect(mocs, "Expected user MOC balance").to.be.bignumber.equal(expectedBalance)
            ...
            expect(mocs, "User MOC balance").to.be.bignumber.equal(EXPECTED_BALANCE.add(ROUNDING_ERROR))
