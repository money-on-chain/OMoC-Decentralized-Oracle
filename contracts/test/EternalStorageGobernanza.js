...
contract('EternalStorageGobernanza', async (accounts) => {
    ...
    before(async () => {
        ...
        await this.eternalStorageGobernanza.setDecimal(
            keys1.decimal,
            values1.decimal.base,
            values1.decimal.exp,
            {from: GOVERNOR},
        );
        await this.eternalStorageGobernanza.setUint(keys1.uInt, values1.uInt, {from: GOVERNOR});
        await this.eternalStorageGobernanza.setString(keys1.string, values1.string, {
            from: GOVERNOR,
        });
        await this.eternalStorageGobernanza.setAddress(keys1.address, values1.address, {
            from: GOVERNOR,
        });
    ...
    it('Should get decimal passing its key', async () => {
        ...
        expect(decimalReturned.base, 'Wrong base number').to.be.bignumber.equal(
            new BN(values1.decimal.base),
        );
        expect(decimalReturned.exp, 'Wrong exp number').to.be.bignumber.equal(
            new BN(values1.decimal.exp),
        );
    });
    ...
    it('Should set string passing its key and with authorized changer', async () => {
        await this.eternalStorageGobernanza.setString(keys2.string, values2.string, {
            from: GOVERNOR,
        });
    ...
    it('Should set address passing its key and with authorized changer', async () => {
        await this.eternalStorageGobernanza.setAddress(keys2.address, values2.address, {
            from: GOVERNOR,
        });
    ...
    it('Should not set decimal passing its key and with unauthorized changer', async () => {
        await expectRevert(
            this.eternalStorageGobernanza.setDecimal(
                keys2.decimal,
                values2.decimal.base,
                values2.decimal.exp,
                {from: NOT_A_GOVERNOR},
    ...
    it('Should not set string passing its key and with unauthorized changer', async () => {
        await expectRevert(
            this.eternalStorageGobernanza.setString(keys2.string, values2.string, {
                from: NOT_A_GOVERNOR,
            }),
            'Invalid changer -- Reason given: Invalid changer.',
        );
    ...
    it('Should not set address passing its key and with unauthorized changer', async () => {
        await expectRevert(
            this.eternalStorageGobernanza.setAddress(keys2.address, values2.address, {
                from: NOT_A_GOVERNOR,
            }),
            'Invalid changer -- Reason given: Invalid changer.',
        );
    ...
    it('Should not set bytes passing its key and with unauthorized changer', async () => {
        await expectRevert(
            this.eternalStorageGobernanza.setBytes(keys2.bytes, values2.bytes, {
                from: NOT_A_GOVERNOR,
            }),
            'Invalid changer -- Reason given: Invalid changer.',
        );
