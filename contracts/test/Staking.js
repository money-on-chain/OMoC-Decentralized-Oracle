...
contract("OracleManager", async (accounts) => {
    ...
    before(async () => {
        ...
        await this.supporters.initialize(this.governor.addr, [this.oracleMgr.address, this.staking.address],
            this.token.address, period);
        await this.oracleMgr.initialize(this.governor.addr, minCPSubscriptionStake, this.supporters.address);
        await this.staking.initialize(this.governor.addr, this.supporters.address,
            this.oracleMgr.address, this.mockDelayMachine.address);

        await this.governor.registerCoinPair(this.oracleMgr, web3.utils.asciiToHex("BTCUSD"),
            this.coinPairPrice_BTCUSD.address);
        await this.governor.registerCoinPair(this.oracleMgr, web3.utils.asciiToHex("RIFBTC"),
    ...
    it("Should register Oracles A, B, C", async () => {
        await this.staking.registerOracle(oracleData[0].account, oracleData[0].name, {from: oracleData[0].owner});
        await this.staking.registerOracle(oracleData[1].account, oracleData[1].name, {from: oracleData[1].owner});
        await this.staking.registerOracle(oracleData[2].account, oracleData[2].name, {from: oracleData[2].owner});
    ...
    it("Should subscribe Oracles A, B, C to coin pair BTCUSD", async () => {
        await this.staking.subscribeToCoinPair(oracleData[0].account, web3.utils.asciiToHex("BTCUSD"), {from: oracleData[0].owner});
        assert.isTrue(await this.coinPairPrice_BTCUSD.isSubscribed(oracleData[0].account));

        await this.staking.subscribeToCoinPair(oracleData[1].account, web3.utils.asciiToHex("BTCUSD"), {from: oracleData[1].owner});
        assert.isTrue(await this.coinPairPrice_BTCUSD.isSubscribed(oracleData[1].account));

        await this.staking.subscribeToCoinPair(oracleData[2].account, web3.utils.asciiToHex("BTCUSD"), {from: oracleData[2].owner});
