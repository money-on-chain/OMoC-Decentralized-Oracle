...
contract("CoinPairPrice", async (accounts) => {
    ...
    it("Should deposit fees from approved account", async () => {
        ...
        assert.equal(await this.coinPairPrice.getAvailableRewardFees(), oldFees.add(FEES).toString());
    });
    ...
    it("Should fail to publish price before the first switch-round call", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s3.r, s2.r, s1.r],
            [s3.s, s2.s, s1.s],
            {from: oracleData[0].account}), "Round not open");
    });
    ...
    it("Should fail to publish with mismatching coinpair", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex(msg.coinpair), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s3.r, s2.r, s1.r],
            [s3.s, s2.s, s1.s],
            {from: oracleData[0].account}), "Coin pair - contract mismatch");
    });
    ...
    it("Should fail to publish with zero price", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s3.r, s2.r, s1.r],
            [s3.s, s2.s, s1.s],
            {from: oracleData[0].account}), "Price must be positive and non-zero");
    });
    ...
    it("Should fail to publish with non-V3 format", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s3.r, s2.r, s1.r],
            [s3.s, s2.s, s1.s],
            {from: oracleData[0].account}), "This contract accepts only V3 format");
    });
    ...
    it("Should fail to publish with inconsistent signature count", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s3.r],
            [s3.s, s2.s, s1.s],
            {from: oracleData[0].account}), "Inconsistent signature count");
    });
    ...
    it("Should fail to publish price if sender is not a voted oracle", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s1.r, s2.r, s3.r],
            [s1.s, s2.s, s3.s],
            {from: oracleData[2].account}), "Your address does not match the voted oracle");
    });

    it("Should fail to publish price if block does not match where the last publication occurred", async () => {
        const thisBlock = 0;

        const {msg, encMsg} = await helpers.getDefaultEncodedMessage(3, "BTCUSD", (10 ** 18).toString(),
            oracleData[0].account, 0);

        const s1 = ethers.utils.splitSignature(await web3.eth.sign(encMsg, oracleData[0].account));
        const s2 = ethers.utils.splitSignature(await web3.eth.sign(encMsg, oracleData[1].account));
        const s3 = ethers.utils.splitSignature(await web3.eth.sign(encMsg, oracleData[2].account));

        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, 0,
            [s1.v, s2.v, s3.v],
            [s1.r, s2.r, s3.r],
            [s1.s, s2.s, s3.s],
            {from: oracleData[0].account}), "Blocknumber does not match the last publication block");
    });
    ...
    it("Should fail to publish price if signature count is  less than 50% of participating oracles", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s1.r],
            [s1.s],
            {from: oracleData[0].account}), "Signature count must exceed 50% of active oracles");
    });
    ...
    it("Should fail to publish if signatures are not unique", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s1.r, s1.r, s1.r],
            [s1.s, s1.s, s1.s],
            {from: oracleData[0].account}), "Signatures are not unique or not ordered by address");
    })
    ...
    it("Should fail to publish if voter/sender is D (not a selected-in-round oracle)", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s1.r, s2.r, s2.r],
            [s1.s, s2.s, s3.s],
            {from: oracleData[3].account}), "Voter oracle is not part of this round");
    })
    ...
    it("Should fail to publish if any signer is not part of round", async () => {
        ...
        await expectRevert(this.coinPairPrice.publishPrice(msg.version, web3.utils.asciiToHex("BTCUSD"), msg.price, msg.votedOracle, (await this.coinPairPrice.lastPublicationBlock()).toString(),
            ...
            [s1.r, s2.r, s2.r],
            [s1.s, s2.s, s3.s],
            {from: oracleData[0].account}), "Address of signer not part of this round");
    })
    ...
    it("Should switch to new round, distributing rewards and resetting points/flags to zero", async () => {
        ...
        assert.equal(expectBalance1.toString(), (await this.token.balanceOf(oracleData[0].owner)).toString());
        assert.equal(expectBalance2.toString(), (await this.token.balanceOf(oracleData[1].owner)).toString());
        assert.equal(expectBalance3.toString(), (await this.token.balanceOf(oracleData[2].owner)).toString());
    ...
    it("Should exclude from round unsubscribed oracles and let remove after that", async () => {
        ...
        await expectRevert(this.oracleMgr.getOracleRegistrationInfo(oracleData[0].account), "Oracle not registered");
        assert.equal((await this.token.balanceOf(oracleData[0].owner)).toString(), initialBalance.add(info.stake).toString());
    });
