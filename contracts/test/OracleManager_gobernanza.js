...
contract("OracleManager by gobernanza", async (accounts) => {
    ...
    it("Unsubscribe by gobernanza", async () => {
        ...
        const change = await OracleManagerUnsubscribeChange.new(this.oracleMgr.address, oracleData[0].account, this.coinPair);
    ...
    it("Should fail to remove oracle if not called by owner", async () => {
        await expectRevert(this.oracleMgr.removeOracle(oracleData[0].account),
