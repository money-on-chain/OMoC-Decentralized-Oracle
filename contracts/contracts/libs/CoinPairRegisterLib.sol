...
library CoinPairRegisterLib {
    ...
    function _registerCoinPair(CoinPairRegisterData storage self, bytes32 coinPair, address addr) internal
 {
    ...
    function _unRegisterCoinPair(CoinPairRegisterData storage self, bytes32 coinPair, uint256 hint) internal
 {
        require(self.coinPairMap[coinPair] != address(0x0), "This coin pair is already unregistered");
    ...
    /// @param coinPair The bytes32-encoded coinpair string (e.g. BTCUSD)
    /// @param addr The contract address associated to the coinpair.
    function _setCoinPair(CoinPairRegisterData storage self, bytes32 coinPair, address addr) internal
 {
        require(addr != address(0), "Address cannot be zero");
    ...
    function _getContractAddress(CoinPairRegisterData storage self, bytes32 coinpair) internal view returns (address)
 {
    ...
    function _getCoinPairCount(CoinPairRegisterData storage self) internal view returns (uint)
 {
    ...
    function _getCoinPairAtIndex(CoinPairRegisterData storage self, uint256 i) internal view returns (bytes32)
 {
    ...
    function _getCoinPairIndex(CoinPairRegisterData storage self, bytes32 coinPair, uint256 hint)
