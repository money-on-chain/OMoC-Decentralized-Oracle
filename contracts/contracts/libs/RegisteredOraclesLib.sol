...
import {OracleInfoLib} from "./OracleInfoLib.sol";
...
library RegisteredOraclesLib {
    ...
    function initRegisteredOracles(function (address) internal view returns (uint256) getStake)
    ...
    function getByAddr(RegisteredOracles storage _self, address addr)
    internal view returns (OracleInfoLib.OracleRegisterInfo storage) {
        return _self.oracleMap[addr].data;
    }

    function getPrevByAddr(RegisteredOracles storage _self, address prevEntry, address addr)
    internal view returns (address) {
        return _sListFindPrevByAddr(_self, prevEntry, addr);
    }

    function getPrevByStake(RegisteredOracles storage _self, address prevEntry, uint stake)
    internal view returns (address) {
        return _sListFindPrevByStake(_self, prevEntry, stake);
    }

    function add(RegisteredOracles storage _self, address addr, string memory internetName, address prevEntry)
    internal {
        _self.oracleMap[addr] = SList(OracleInfoLib.initRegisterInfo(msg.sender, internetName), address(0));
        _insert(_self, addr, prevEntry);
    }

    function modify(RegisteredOracles storage _self, address addr, address removePrevEntry, address addPrevEntry)
    internal {
    ...
    function _sListFindPrevByStake(RegisteredOracles storage _self, address prevEntry, uint stake)
    private view returns (address) {
        ...
        for (address currlm = _self.oracleMap[prev].sle_next;
            currlm != address(0);
            currlm = _self.oracleMap[currlm].sle_next)
     {
    ...
    // TODO: Implement a partial search with some gas limit
    function _sListFindPrevByAddr(RegisteredOracles storage _self, address prevEntry, address addr)
    private view returns (address) {
