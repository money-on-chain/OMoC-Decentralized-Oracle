...
library RegisteredOraclesLib {
    ...
    function getNext(RegisteredOracles storage _self, address iterator)
        internal
        view
        returns (address)
    {
    ...
    function getByAddr(RegisteredOracles storage _self, address addr)
        internal
        view
        returns (OracleInfoLib.OracleRegisterInfo storage)
    {
        return _self.oracleMap[addr].data;
    }

    function getPrevByAddr(
        RegisteredOracles storage _self,
        address prevEntry,
        address addr
    ...
    function add(
        ...
        _self.oracleMap[addr] = SList(
            OracleInfoLib.initRegisterInfo(msg.sender, internetName),
            address(0)
        );
    ...
    function modify(
        RegisteredOracles storage _self,
        address addr,
        address removePrevEntry,
        address addPrevEntry
    ) internal {
    ...
    function _sListFindPrevByStake(
        RegisteredOracles storage _self,
        address prevEntry,
        uint256 stake
        ...
        for (
            address currlm = _self.oracleMap[prev].sle_next;
            currlm != address(0);
