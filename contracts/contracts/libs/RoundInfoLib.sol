...
library RoundInfoLib {
    ...
    */
    function initRoundInfo(uint256 _maxOraclesPerRound, uint256 _roundLockPeriodInBlocks, uint8 _numIdleRounds)
    internal pure returns (RoundInfo memory) {
        ...
        return RoundInfo(0, 0, 0, 0, _maxOraclesPerRound, _roundLockPeriodInBlocks, _numIdleRounds, new address[](0));
    }
    ...
    function isInCurrentRound(RoundInfo storage _self, address _oracleAddr) internal view returns (bool) {
        return _self.oracleRoundInfo[_oracleAddr].isInRound(_self.number);
    }

    function canRemoveOracle(RoundInfo storage _self, address _oracleAddr) internal view returns (bool) {
    ...
    function addPoints(RoundInfo storage _self, address _oracleAddr, uint _points) internal {
    ...
    internal view returns (uint points, uint256 selectedInRound, bool selectedInCurrentRound) {
        return (
    ...
    function getRoundInfo(RoundInfo storage _self) internal view returns (uint256 round,
        uint256 startBlock,
        uint256 lockPeriodEndBlock,
        uint256 totalPoints,
        address[] memory selectedOracles) {
        return (_self.number, _self.startBlock, _self.lockPeriodEndBlock, _self.totalPoints, _self.selectedOracles);
    }

    function getPoints(RoundInfo storage _self, address _oracleAddr) internal view returns (uint points) {
