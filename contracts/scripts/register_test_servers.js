...
async function main() {
    ...
    if (!oracles) {
        console.error(
            'Please set the right network id',
            Object.keys(ORACLES_TO_REGISTER_BY_NETWORK),
        );
        process.exit();
    }
    ...
    const quantity = oracles
        .filter((x) => is_registered[x.addr] === false)
        .reduce((acc, x) => acc.add(x.quantity), toBN('0'));
    ...
    for (const o of oracles) {
        if (is_registered[o.addr] !== false) {
            ...
            console.log(
                oracleManager.address,
                'Calling registerOracle ',
                o.addr,
                o.name,
                o.quantity.toString(),
            );
            await oracleManager.registerOracle(o.addr, o.name, o.quantity, {
                from: owner,
                gas: 4000000,
            });
        }
        for (const coinPair of o.oracleCoinPairFilter) {
            const isSubscribed = await oracleManager.isSubscribed(
                o.addr,
                web3.utils.fromAscii(coinPair),
            );
            if (isSubscribed) {
