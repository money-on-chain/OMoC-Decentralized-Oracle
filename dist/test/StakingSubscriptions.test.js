...
describe('Staking-subscriptions', function () {
    ...
    beforeEach(async function () {
        ...
        if (accounts.length < oracleStartIndex + NUM_ORACLES) {
            throw new Error(`Expected at least ${oracleStartIndex + NUM_ORACLES} wallet clients, got ${accounts.length}`);
        }
        ...
        await contracts.governor.mint(contracts.token.address, accounts[mocAccountIndex].account.address, MAX_STAKE);
    ...
    async function prepareOracle(entry) {
    ...
    it('subscription - replacing low stakes', async function () {
        ...
        for (let i = Number(MAX_SUBSCRIBED_ORACLES); i < NUM_ORACLES; i += 1) {
            ...
            expect(await coinPairPrice.read.isSubscribed([toRemove.owner.account.address])).to.equal(true);
            expect(await coinPairPrice.read.isSubscribed([oracle.owner.account.address])).to.equal(false);
            await contracts.staking.write.subscribeToCoinPair([coinPairId], {
                account: oracle.owner.account,
            });
            expect(await coinPairPrice.read.isSubscribed([oracle.owner.account.address])).to.equal(true);
            expect(await coinPairPrice.read.isSubscribed([toRemove.owner.account.address])).to.equal(false);
        }
    });
    it('removing oracle', async function () {
        ...
        assert.equal((await contracts.staking.read.getBalance([oracle.owner.account.address])).toString(), '0');
        expect(await contracts.oracleMgr.read.canRemoveOracle([oracle.owner.account.address])).to.equal(true);
...
//# sourceMappingURL=StakingSubscriptions.test.js.map