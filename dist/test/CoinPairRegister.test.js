...
describe('CoinPairRegister', function () {
    ...
    before(async function () {
        ...
        coinPairRegister = {
            ...
            registerCoinPair: async (coinPair, address) => {
                return governor.registerCoinPair(oracleManager, coinPair, address);
            },
        };
    });
    it('must register BTCUSD coin pair', async function () {
        await coinPairRegister.registerCoinPair(encodeCoinPair(pairs[0].coinPair), pairs[0].address);
    });
    it('must register BTCRIF coin pair', async function () {
        await coinPairRegister.registerCoinPair(encodeCoinPair(pairs[1].coinPair), pairs[1].address);
    });
    it('must fail to register BTCRIF coin pair twice', async function () {
        await viem.assertions.revertWith(coinPairRegister.registerCoinPair(encodeCoinPair(pairs[1].coinPair), pairs[1].address), 'Pair is already registered');
    });
    it('must fail to register if called by non-governance account', async function () {
        await viem.assertions.revertWith(coinPairRegister.contract.write.registerCoinPair([encodeCoinPair(pairs[1].coinPair), pairs[1].address], { account: accounts[2].account }), 'not_authorized_changer');
    });
    it('must fail to register coin pair with address zero', async function () {
        await viem.assertions.revertWith(coinPairRegister.registerCoinPair(encodeCoinPair('ZERO'), '0x0000000000000000000000000000000000000000'), 'Address cannot be zero');
    });
    it('must retrieve the number of registered coin pairs', async function () {
        expect(await coinPairRegister.contract.read.getCoinPairCount()).to.equal(BigInt(pairs.length));
    });
    it('must retrieve the correct coin pair at index', async function () {
        expect(decodeCoinPair(await coinPairRegister.contract.read.getCoinPairAtIndex([0n]))).to.equal(pairs[0].coinPair);
        expect(decodeCoinPair(await coinPairRegister.contract.read.getCoinPairAtIndex([1n]))).to.equal(pairs[1].coinPair);
    });
    it('must fail if out of bounds index', async function () {
        await viem.assertions.revertWith(coinPairRegister.contract.read.getCoinPairAtIndex([BigInt(pairs.length) + 10n]), 'Illegal index');
    });
});
//# sourceMappingURL=CoinPairRegister.test.js.map