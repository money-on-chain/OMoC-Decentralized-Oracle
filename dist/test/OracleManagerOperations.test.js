...
describe('OracleManager operations', function () {
    ...
    let accounts;
    let contracts;
    async function createMockGovernor(owner) {
    ...
    async function createContractsWithMockGovernor(governorOwner) {
    ...
    async function registerOracle(oracleAddress = ADDRESS_ONE, name = 'ORACLE-A') {
        await contracts.oracleMgr.write.registerOracle([accounts[ORACLE_OWNER].account.address, oracleAddress, name], { account: accounts[GOVERNOR_OWNER].account });
    }
    beforeEach(async function () {
        ...
        await contracts.token.write.mint([accounts[ORACLE_OWNER].account.address, MIN_ORACLE_STAKE], {
            account: accounts[GOVERNOR_OWNER].account,
        });
        await contracts.token.write.approve([contracts.staking.address, MIN_ORACLE_STAKE], {
            account: accounts[ORACLE_OWNER].account,
        });
        await contracts.staking.write.deposit([MIN_ORACLE_STAKE, accounts[ORACLE_OWNER].account.address], { account: accounts[ORACLE_OWNER].account });
    });
    it('Whitelist manipulation', async function () {
        await viem.assertions.revertWith(contracts.oracleMgr.write.addToWhitelist([accounts[2].account.address]), 'Invalid changer');
        await contracts.oracleMgr.write.addToWhitelist([accounts[2].account.address], {
            account: accounts[GOVERNOR_OWNER].account,
        });
        await viem.assertions.revertWith(contracts.oracleMgr.write.removeFromWhitelist([accounts[2].account.address]), 'Invalid changer');
    ...
    it('Change oracle address', async function () {
        ...
        expect(await contracts.oracleMgr.read.isRegistered([accounts[ORACLE_OWNER].account.address])).to.equal(true);
        const newAddress = addressFromNumber(10);
        await contracts.oracleMgr.write.setOracleAddress([accounts[ORACLE_OWNER].account.address, newAddress], { account: accounts[GOVERNOR_OWNER].account });
    ...
    it('Fail to change oracle address', async function () {
        ...
        await viem.assertions.revertWith(contracts.oracleMgr.write.setOracleAddress([ADDRESS_ONE, newAddress], {
            account: accounts[GOVERNOR_OWNER].account,
        }), 'Oracle not registered');
    });
    it('Oracle is not registered', async function () {
        expect(await contracts.oracleMgr.read.isSubscribed([ADDRESS_ONE, COINPAIR_ID])).to.equal(false);
    });
    ...
    it('Remove oracle', async function () {
        await registerOracle();
        expect(await contracts.oracleMgr.read.isRegistered([accounts[ORACLE_OWNER].account.address])).to.equal(true);
        await contracts.oracleMgr.write.subscribeToCoinPair([accounts[ORACLE_OWNER].account.address, COINPAIR_ID], { account: accounts[GOVERNOR_OWNER].account });
        expect(await contracts.oracleMgr.read.isSubscribed([
            accounts[ORACLE_OWNER].account.address,
            COINPAIR_ID,
        ])).to.equal(true);
        ...
        expect(await contracts.staking.read.getBalance([accounts[ORACLE_OWNER].account.address])).to.equal(0n);
        await contracts.oracleMgr.write.removeOracle([accounts[ORACLE_OWNER].account.address], {
            account: accounts[GOVERNOR_OWNER].account,
        });
        expect(await contracts.oracleMgr.read.isRegistered([accounts[ORACLE_OWNER].account.address])).to.equal(false);
    });
    it('Initialization verifications', async function () {
        const oracleManager = await deployer.deployUninitializedProxy('OracleManager');
        await viem.assertions.revertWith(oracleManager.write.initialize([
            contracts.governor.address,
            MIN_ORACLE_STAKE,
            ADDRESS_ZERO,
            [],
        ]), 'Staking contract address must be != 0');
        await viem.assertions.revertWith(oracleManager.write.initialize([contracts.governor.address, 0n, ADDRESS_ONE, []]), 'The minimum coin pair subscription stake amount cannot be zero');
    });
});
//# sourceMappingURL=OracleManagerOperations.test.js.map