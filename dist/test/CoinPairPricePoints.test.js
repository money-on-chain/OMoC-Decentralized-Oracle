...
import { getDefaultEncodedMessage, initCoinpair, initContracts, mineUntilNextRound, } from '../src/helpers.js';
import { assertAddressInList, Deployer, } from 'ts-test-helpers';
...
describe('CoinPairPrice distribution', function () {
    async function buildFixture(maxOraclesPerRound = 4n) {
        const { viem, networkHelpers } = await network.create();
        const deployer = await Deployer.default(viem);
        const accounts = await viem.getWalletClients();
        const contracts = await initContracts(deployer, accounts[8]);
        const coinPairPrice = await initCoinpair(deployer, coinPairName, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], maxOraclesPerRound, 30n, 60n, 0n, 3n);
        const oracleData = [
            {
                name: 'oracle-a.io',
                stake: 4n * 10n ** 18n,
        ...
        for (const index of [0, 2, 4, 6, 8]) {
            await contracts.governor.mint(contracts.token.address, accounts[index].account.address, initialBalance);
        }
    ...
    async function registerAndSubscribe(fixture, count) {
        const coinPair = await fixture.coinPairPrice.read.getCoinPair();
        for (const oracle of fixture.oracleData.slice(0, count)) {
            await fixture.contracts.token.write.approve([fixture.contracts.staking.address, oracle.stake], { account: oracle.owner.account });
            await fixture.contracts.staking.write.registerOracle([oracle.address, oracle.name], {
                account: oracle.owner.account,
            });
            await fixture.contracts.staking.write.deposit([oracle.stake, oracle.owner.account.address], { account: oracle.owner.account });
            await fixture.contracts.staking.write.subscribeToCoinPair([coinPair], {
                account: oracle.owner.account,
            });
        }
    }
    async function addFees(fixture) {
        ...
        expect(await fixture.coinPairPrice.read.getAvailableRewardFees()).to.equal(oldFees + feeAmount);
    }
    async function publishOnce(fixture, publisherIndex, signerCount) {
        ...
        const { msg, encMsg } = await getDefaultEncodedMessage(3, coinPairName, price, publisher.address, lastPublicationBlock);
        ...
        const signatures = await Promise.all(signers.map(async (oracle) => parseSignature(await oracle.account.signMessage({
    ...
    async function publishRepeatedly(fixture, publisherIndex, times, signerCount) {
        for (let i = 0; i < times; i += 1) {
            await publishOnce(fixture, publisherIndex, signerCount);
        }
    }
    async function expectRewardDistribution(fixture, participatingOwners, totalPointsOverride) {
        const { coinPairPrice, contracts, oracleData } = fixture;
        const sourceBalance = await coinPairPrice.read.getAvailableRewardFees();
        const ownerBalances = await Promise.all(participatingOwners.map((index) => contracts.token.read.balanceOf([oracleData[index].owner.account.address])));
        const ownerPoints = await Promise.all(participatingOwners.map(async (index) => {
            const info = await coinPairPrice.read.getOracleRoundInfo([
                oracleData[index].owner.account.address,
            ]);
            return info[0];
        }));
        const totalPoints = totalPointsOverride ?? ownerPoints.reduce((acc, points) => acc + points, 0n);
        ...
        for (const [position, oracleIndex] of participatingOwners.entries()) {
            expect(await contracts.token.read.balanceOf([
                oracleData[oracleIndex].owner.account.address,
            ])).to.equal(ownerBalances[position] + expectedRewards[position]);
        }
        expect(await coinPairPrice.read.getAvailableRewardFees()).to.equal(sourceBalance - expectedTotalReward);
    }
    ...
    describe('Test with maxOraclesPerRound equal to 3', function () {
        it("Owner is replaced in round for another one and doesn't receive the points", async function () {
            ...
            const ownerPoints3BeforeLeavingRound = (await fixture.coinPairPrice.read.getOracleRoundInfo([
            ...
            await fixture.networkHelpers.mine();
            const ownerPoints1 = (await fixture.coinPairPrice.read.getOracleRoundInfo([
                fixture.oracleData[0].owner.account.address,
            ]))[0];
            const ownerPoints2 = (await fixture.coinPairPrice.read.getOracleRoundInfo([
...
//# sourceMappingURL=CoinPairPricePoints.test.js.map