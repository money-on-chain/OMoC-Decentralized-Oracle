...
describe('DelayMachine', function () {
    ...
    let accounts;
    let governorOwner;
    let destinationMocHolder;
    let sourceMocHolder;
    let dummyAccount;
    const INITIAL_BALANCE = 100000000000000000000n;
    function notExpired(expiration, currentTime, expirationDelta) {
    ...
    async function depositViaStaking(contract, token, staking, amount, expirationSecs) {
        await token.write.approve([staking.address, amount], {
            account: sourceMocHolder.account,
        });
        await staking.write.depositFrom([amount, destinationMocHolder.account.address, sourceMocHolder.account.address], {
            account: sourceMocHolder.account,
        });
    ...
    describe('direct operation', function () {
        ...
        let token;
        beforeEach(async function () {
            ({ erc20: token, delay: contract } = await createContracts(sourceMocHolder.account.address));
        });
        it('creation', async function () {
            expect(await token.read.balanceOf([sourceMocHolder.account.address])).to.equal(INITIAL_BALANCE);
            await token.write.approve([contract.address, 123n], {
                account: sourceMocHolder.account,
            });
            expect(await token.read.allowance([sourceMocHolder.account.address, contract.address])).to.equal(123n);
        });
        it('deposit fail', async function () {
            await viem.assertions.revertWith(contract.write.deposit([250n, destinationMocHolder.account.address, 3600n], {
        ...
        it('deposit', async function () {
            ...
            const tx = await contract.write.deposit([amount, destinationMocHolder.account.address, expirationSecs], { account: sourceMocHolder.account });
            expect(await contract.read.getBalance([destinationMocHolder.account.address])).to.equal(amount);
            ...
            expect(notExpired(expirations[0], BigInt(latest.timestamp), expirationSecs)).to.equal(true);
        ...
        it('withdraw fail', async function () {
            ...
            await contract.write.deposit([amount, destinationMocHolder.account.address, expirationSecs], {
                account: sourceMocHolder.account,
            });
            await viem.assertions.revertWith(contract.write.withdraw([1n], { account: sourceMocHolder.account }), 'Invalid ID');
            await viem.assertions.revertWith(contract.write.withdraw([1n], { account: destinationMocHolder.account }), 'Not expired');
        });
    ...
    describe('stacking machine mock operation', function () {
        ...
        it('deposit', async function () {
            ...
            expect(await contract.read.getBalance([destinationMocHolder.account.address])).to.equal(250n);
        });
        it('withdraw fail', async function () {
            const payId = await depositViaStaking(contract, token, staking, 250n, 3600n);
            await viem.assertions.revertWith(contract.write.withdraw([payId], { account: sourceMocHolder.account }), 'Invalid ID');
            await viem.assertions.revertWith(contract.write.withdraw([payId], { account: destinationMocHolder.account }), 'Not expired');
        });
        it('withdraw success', async function () {
            const amount = 250n;
        ...
        it('cancel fail', async function () {
            const payId = await depositViaStaking(contract, token, staking, 250n, 3600n);
            await viem.assertions.revertWith(contract.write.cancel([payId], { account: sourceMocHolder.account }), 'Invalid ID');
        });
        it('cancel success', async function () {
            ...
            assertSameAddress(e.destination, destinationMocHolder.account.address);
            expect(e.amount).to.eq(amount);
            assertSameAddress(await staking.read.destination(), destinationMocHolder.account.address);
    ...
    describe('multiple deposits [slow] [skip-on-coverage]', function () {
        ...
        it('withdraw', async function () {
            ...
            for (let i = 0; i < expirations.length; i += 1) {
                const id = await depositViaStaking(contract, token, staking, amounts[i], expirations[i]);
                total += amounts[i];
                expect(await contract.read.getBalance([destinationMocHolder.account.address])).to.equal(total);
            ...
            for (let i = 0; i < inserted.length; i += 1) {
                ...
                for (let j = 0; j < inserted.length; j += 1) {
                    if (j < i) {
                        await viem.assertions.revertWith(contract.write.withdraw([inserted[j].id], {
                            account: destinationMocHolder.account,
                        }), 'Invalid ID');
                    }
                    else if (j === i) {
...
//# sourceMappingURL=DelayMachine.test.js.map