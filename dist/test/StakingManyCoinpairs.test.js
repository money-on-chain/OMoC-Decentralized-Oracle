...
describe('Staking-withdraw with many coin pairs', function () {
    ...
    before(async function () {
        ...
        for (let i = 0; i < NUM_COINPAIR; i += 1) {
            const coinPair = await initCoinpair(deployer, `${COINPAIR_NAME}${i}`, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[governorOwnerIndex].account.address], MAX_SELECTED_ORACLES, BigInt(NUM_SUBSCRIBED_ORACLES));
            coinPairs.push(coinPair);
        }
        await contracts.governor.mint(contracts.token.address, accounts[feesAccountIndex].account.address, 100n * 10n ** 18n);
        ...
        for (const oracle of oracles) {
            ...
            await contracts.staking.write.registerOracle([oracle.oracle.account.address, oracle.name], {
                account: oracle.owner.account,
            });
    ...
    it('subscription', async function () {
        for (const oracle of oracles) {
            for (const coinPair of coinPairs) {
                await contracts.staking.write.subscribeToCoinPair([await coinPair.read.getCoinPair()], {
                    account: oracle.owner.account,
                });
            }
        }
...
//# sourceMappingURL=StakingManyCoinpairs.test.js.map