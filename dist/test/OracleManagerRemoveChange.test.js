...
describe('OracleManagerRemoveChange', function () {
    it('should succeed execute call', async function () {
        ...
        expect(await contracts.oracleMgr.read.isOracleRegistered([oracleOwner.account.address])).to.equal(true);
        await contracts.governor.execute(change);
        expect(await contracts.oracleMgr.read.isOracleRegistered([oracleOwner.account.address])).to.equal(false);
    });
});
//# sourceMappingURL=OracleManagerRemoveChange.test.js.map