...
describe('CoinPairPrice Signature', function () {
    async function setup(cantOracles) {
    ...
    async function register(contracts, coinPairPrice, oracleData, cantOracles) {
    ...
    async function signWithOwner(coinPairPrice, oracleData, cantSignatures) {
        const sender = oracleData[0].account;
        const thisCoinPair = await coinPairPrice.read.getCoinPair();
        const lastPubBlock = await coinPairPrice.read.getLastPublicationBlock();
        const { msg, encMsg } = await getDefaultEncodedMessage(3, 'BTCUSD', 10n ** 18n, sender.account.address, lastPubBlock);
        const signatures = [
            parseSignature(await sender.signMessage({
        ...
        for (let i = 0; i < cantSignatures; i += 1) {
            ...
            signatures.push(parseSignature(await signer.signMessage({
    ...
    for (const testGroup of testsToRun) {
        describe(`Test for ${testGroup.oracles} oracles`, function () {
            for (const test of testGroup.tests) {
                ...
                const reason = testGroup.oracles < 3
...
//# sourceMappingURL=CoinPairPriceSignatures.test.js.map