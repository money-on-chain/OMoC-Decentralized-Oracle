...
import { assertAddressInList, assertAddressNotInList, Deployer, } from 'ts-test-helpers';
const coinPairName = 'BTCUSD';
const maxOraclesPerRound = 10n;
const maxSubscribedOraclesPerRound = 20n;
const minSubscriptionStake = 10000000000n;
...
describe('CoinPairPrice Min Stake', function () {
    ...
    async function buildFixture() {
        ...
        const coinPairPrice = await initCoinpair(deployer, coinPairName, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], maxOraclesPerRound, maxSubscribedOraclesPerRound);
    ...
    async function mintAndDeposit(contracts, owner, stake) {
        const initialBalance = await contracts.token.read.balanceOf([owner.account.address]);
        await contracts.governor.mint(contracts.token.address, owner.account.address, initialMintAmount);
        ...
        expect(await contracts.token.read.balanceOf([owner.account.address])).to.equal(initialBalance + initialMintAmount - stake);
    }
    describe('Min subscription stake', function () {
        it('creation', async function () {
            const { contracts } = await buildFixture();
            await contracts.governor.mint(contracts.token.address, accounts[4].account.address, initialMintAmount);
        });
        it('Should fail to subscribe with less than minSubscriptionStake', async function () {
            ...
            await contracts.staking.write.registerOracle([oracleAddr.account.address, 'not enough stake'], {
                account: ownerAccount.account,
            });
            await mintAndDeposit(contracts, ownerAccount, stake);
            await viem.assertions.revertWith(contracts.staking.write.subscribeToCoinPair([await coinPairPrice.read.getCoinPair()], {
                account: ownerAccount.account,
            }), 'Not enough stake');
        });
    });
    describe('After subscription if we have less than minSubscriptionStake we are kicked off', function () {
        it('creation', async function () {
            const { contracts } = await buildFixture();
            await contracts.governor.mint(contracts.token.address, oracles[0].owner.account.address, initialMintAmount);
        });
        it('subscribe other', async function () {
            ...
            for (let i = 1; i < oracles.length; i += 1) {
                ...
                expect(await contracts.staking.read.getBalance([oracle.owner.account.address])).to.equal(minSubscriptionStake);
            }
        ...
        it('publish', async function () {
            ...
            expect((await coinPairPrice.read.getOracleRoundInfo([oracles[0].owner.account.address]))[0]).to.equal(1n);
        });
        it('withdraw', async function () {
            ...
            expect(await contracts.staking.read.getBalance([oracles[0].owner.account.address])).to.equal(minSubscriptionStake - 1n);
            expect(await contracts.oracleMgr.read.isSubscribed([
                oracles[0].owner.account.address,
                coinPair,
            ])).to.equal(false);
            assertAddressNotInList((await coinPairPrice.read.getRoundInfo())[5], oracles[0].address);
            expect((await coinPairPrice.read.getOracleRoundInfo([oracles[0].owner.account.address]))[0]).to.equal(0n);
        });
    });
});
//# sourceMappingURL=CoinPairPriceMinStake.test.js.map