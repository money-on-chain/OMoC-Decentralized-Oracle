...
describe('CoinPairPriceFree', function () {
    ...
    it('coinPairPriceFree', async function () {
        ...
        const coinPairPrice = await initCoinpair(deployer, COINPAIR_NAME, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address, coinPairPriceFree.address], 3n, 3n);
        ...
        for (const { address, name, owner } of oracles) {
            ...
            await contracts.governor.mint(contracts.token.address, owner.account.address, ORACLE_STAKE);
        ...
        await viem.assertions.revertWith(coinPairPrice.read.peek(pubAccount), 'Address is not whitelisted');
        ...
        expect(await coinPairPriceFree.read.getLastPublicationBlock(pubAccount)).to.equal(lastPublicationBlock);
...
//# sourceMappingURL=CoinPairPriceFree.test.js.map