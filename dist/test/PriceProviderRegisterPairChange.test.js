...
describe('PriceProviderRegisterPairChange', function () {
    it('Should succeed execute call', async function () {
        ...
        assertSameAddress(await priceProviderRegister.read.getContractAddress([encodeCoinPair('BTCUSD')]), contracts.coinPairPriceBTCUSD.address);
        assertSameAddress(await priceProviderRegister.read.getContractAddress([encodeCoinPair('RIFBTC')]), contracts.coinPairPriceRIFBTC.address);
        expect(await priceProviderRegister.read.getCoinPairCount()).to.equal(2n);
    });
});
//# sourceMappingURL=PriceProviderRegisterPairChange.test.js.map