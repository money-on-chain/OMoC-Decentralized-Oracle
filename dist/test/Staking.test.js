...
describe('Staking', function () {
    ...
    let accounts;
    let contracts;
    let coinPairPriceBtcusd;
    let coinPairPriceRifbtc;
    ...
    beforeEach(async function () {
        ...
        contracts = await initContracts(deployer, accounts[governorOwnerIndex], period, minCPSubscriptionStake);
        coinPairPriceBtcusd = await initCoinpair(deployer, 'BTCUSD', contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address]);
        coinPairPriceRifbtc = await initCoinpair(deployer, 'RIFBTC', contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address]);
        for (const idx of [0, 2, 4, 6, 8]) {
            await contracts.governor.mint(contracts.token.address, accounts[idx].account.address, 800n * 10n ** 18n);
        }
    });
    async function registerOracle(entry) {
    ...
    async function depositOracle(entry) {
        const owner = accounts[entry.owner];
    ...
    describe('Should register Oracles A, B, C', function () {
        it('registers the first three oracles', async function () {
            ...
            for (const entry of oracleData.slice(0, 3)) {
                ...
                expect(await contracts.staking.read.isOracleRegistered([
                    accounts[entry.owner].account.address,
                ])).to.equal(true);
            }
            ...
            for (let idx = 0; idx < Number(count); idx += 1) {
                registered.push(await contracts.staking.read.getRegisteredOracleAtIndex([BigInt(idx)]));
            }
            expect(registered.map((o) => getAddress(o[0]))).to.have.members(oracleData.slice(0, 3).map((x) => getAddress(accounts[x.owner].account.address)));
        });
    });
    describe('Should subscribe Oracles A, B, C to coin pair BTCUSD', function () {
        it('subscribes the first three oracles', async function () {
            for (const entry of oracleData.slice(0, 3)) {
                ...
                await contracts.staking.write.subscribeToCoinPair([await coinPairPriceBtcusd.read.getCoinPair()], {
                    account: accounts[entry.owner].account,
                });
                expect(await coinPairPriceBtcusd.read.isSubscribed([
                    accounts[entry.owner].account.address,
                ])).to.equal(true);
            }
        });
    ...
    describe("Should not be able to lock mocs from an address other than the voting machine's", function () {
        it('reverts for direct calls', async function () {
            untilTimestampLock = BigInt(Math.round(Date.now() / 1000) + Number(secsUntilStakeRelease));
            await viem.assertions.revertWith(contracts.staking.write.lockMocs([accounts[oracleData[1].owner].account.address, untilTimestampLock], { account: accounts[oracleData[1].owner].account }), 'Address is not whitelisted');
        });
    });
    describe('Should lock stake of oracle B', function () {
        it('locks oracle B stake through the voting machine', async function () {
            ...
            await contracts.votingMachine.write.lockMocs([accounts[oracleData[1].owner].account.address, untilTimestampLock], { account: accounts[oracleData[1].owner].account });
    ...
    describe('Should not be able to withdraw stake of oracle B until it is unlocked', function () {
        it('waits until unlock time', async function () {
            ...
            await depositOracle(oracleData[1]);
            await contracts.votingMachine.write.lockMocs([accounts[oracleData[1].owner].account.address, untilTimestampLock], { account: accounts[oracleData[1].owner].account });
            await viem.assertions.revertWith(contracts.staking.write.withdraw([oracleData[1].stake], {
    ...
    describe('Should not be able to withdraw stake of oracle D because it has none', function () {
        it('reverts on empty balance', async function () {
            ...
            await viem.assertions.revertWith(contracts.staking.write.withdraw([oracleData[3].stake], {
    ...
    describe('Coin pair smoke', function () {
        it('created both coin pairs', async function () {
            expect(await coinPairPriceBtcusd.read.getCoinPair()).to.equal(await coinPairPriceBtcusd.read.getCoinPair());
            expect(await coinPairPriceRifbtc.read.getCoinPair()).to.equal(await coinPairPriceRifbtc.read.getCoinPair());
        });
    });
});
