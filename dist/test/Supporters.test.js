...
import { createGovernor, getLatestBlock, initContractsWithCoinPairs, waitForEvents, } from '../src/helpers.js';
...
describe('Supporters', function () {
    ...
    async function deploySupporters(governorAddress, whitelist = []) {
    ...
    describe('Creation', function () {
        ...
        it('must fail for addresses that are not whitelisted', async function () {
            ...
            await viem.assertions.revertWith(supporters.write.stakeAt([BALANCE_USER1, user3.account.address], {
                account: user2.account,
            }), 'Address is not whitelisted');
            await viem.assertions.revertWith(supporters.write.withdrawFrom([BALANCE_USER1, user3.account.address], {
                account: user2.account,
            }), 'Address is not whitelisted');
        });
    });
    describe('Subaccounts', function () {
        ...
        it('stake', async function () {
            ...
            expect(await supporters.read.getBalanceAt([
    ...
    describe('IterableWhiteList', function () {
        ...
        it('add and remove', async function () {
            ...
            expect(await supporters.read.isWhitelisted([accounts[1].account.address])).to.equal(false);
            expect(await supporters.read.isWhitelisted([accounts[4].account.address])).to.equal(true);
            expect(await supporters.read.isWhitelisted([accounts[5].account.address])).to.equal(true);
            await supporters.write.removeFromWhitelist([accounts[4].account.address], {
                account: accounts[governorOwnerIndex].account,
            });
            expect(await supporters.read.isWhitelisted([accounts[4].account.address])).to.equal(false);
        });
    });
    describe('Governance', function () {
        ...
        it('should fail in if not a governor call', async function () {
            await viem.assertions.revertWith(supporters.write.addToWhitelist([accounts[0].account.address], {
    ...
    describe('Distribute', function () {
        ...
        beforeEach(async function () {
            ...
            for (const idx of [0, 2, 4, 6, 8]) {
                await contracts.governor.mint(token.address, accounts[idx].account.address, 800n * 10n ** 18n);
            }
        });
        it('Distribute should fail if contract not ready to distribute', async function () {
            ...
            await viem.assertions.revertWith(supporters.write.distribute({ account: accounts[2].account }), 'Not ready to distribute');
        });
        it('Distribute should succeed if contract is ready to distribute', async function () {
            ...
            await contracts.staking.write.deposit([oracleData[0].stake, oracleData[0].owner.account.address], {
                account: oracleData[0].owner.account,
            });
...
//# sourceMappingURL=Supporters.test.js.map