...
describe('CalculatedPriceProvider', function () {
    ...
    it('after multiply and divide by the same return 1', async function () {
        ...
        const calculatedPriceProvider = await constructCalculatedPriceProvider(123n, [mock1.address], 123n, [mock1.address]);
        const val = await calculatedPriceProvider.read.getPriceInfo();
        expect(val[0]).to.equal(1n);
        expect(val[1]).to.equal(true);
    ...
    it('should fail if any of the source providers fail', async function () {
        ...
        let val = await (await constructCalculatedPriceProvider(123n, [success.address, fail.address], 123n, [
            success.address,
        ])).read.getPriceInfo();
        expect(val[0]).to.equal(0n);
        expect(val[1]).to.equal(false);
        expect(val[2]).to.equal(213n);
        val = await (await constructCalculatedPriceProvider(123n, [success.address], 123n, [
            success.address,
            fail.address,
        ])).read.getPriceInfo();
        expect(val[0]).to.equal(0n);
        expect(val[1]).to.equal(false);
        expect(val[2]).to.equal(213n);
        val = await (await constructCalculatedPriceProvider(123n, [success.address, fail.address], 123n, [
            success.address,
            fail.address,
        ])).read.getPriceInfo();
        expect(val[0]).to.equal(0n);
    ...
    it('some calculation', async function () {
        ...
        const contract = await constructCalculatedPriceProvider(456n, [mock1.address, mock2.address], 789n, [mock1.address], [accounts[0].account.address]);
    ...
    it('lastPubBlock', async function () {
        ...
        for (let i = 1n; i <= 5n; i += 1n) {
            mocks.push((await deployer.deploy('MockIPriceProvider', [546n, true, lastPubBlockMin * i]))
                .address);
            mocks.unshift((await deployer.deploy('MockIPriceProvider', [123n, true, lastPubBlockMin * i]))
                .address);
        }
    ...
    it('whitelist manipulation', async function () {
        const whitelist = [ADDRESS_ONE, accounts[1].account.address];
        const calculatedPriceProvider = await constructCalculatedPriceProvider(1n, [], 1n, [], whitelist);
        expect(await calculatedPriceProvider.read.getWhiteListLen()).to.equal(2n);
        assertSameAddress(await calculatedPriceProvider.read.getWhiteListAtIndex([0n]), whitelist[0]);
        assertSameAddress(await calculatedPriceProvider.read.getWhiteListAtIndex([1n]), whitelist[1]);
        await viem.assertions.revertWith(calculatedPriceProvider.write.addToWhitelist([accounts[2].account.address]), 'Invalid changer');
        ...
        await viem.assertions.revertWith(calculatedPriceProvider.write.removeFromWhitelist([accounts[1].account.address]), 'Invalid changer');
...
//# sourceMappingURL=CalculatedPriceProvider.test.js.map