...
import { assertSameAddress, Deployer, ADDRESS_ZERO, ADDRESS_ONE, } from 'ts-test-helpers';
...
describe('IterableOracles', function () {
    ...
    it('fail registration zero address', async function () {
        await viem.assertions.revertWith(iterableOracles.write.registerOracle([
            ADDRESS_ZERO,
            ADDRESS_ONE,
    ...
    it('registration success', async function () {
        ...
        await viem.assertions.revertWith(iterableOracles.read.getOracleAtIndex([length]), 'Illegal index');
    ...
    it('fail to change url', async function () {
        await viem.assertions.revertWith(iterableOracles.write.setName([ADDRESS_ZERO, 'https://example.org/bad']), 'Oracle owner is not registered');
    });
    ...
    it('fail to change oracle address', async function () {
        ...
        await viem.assertions.revertWith(iterableOracles.write.setOracleAddress([ADDRESS_ZERO, ADDRESS_ONE]), 'Oracle owner is not registered');
        await viem.assertions.revertWith(iterableOracles.write.setOracleAddress([owner.account.address, address]), 'Oracle already registered');
    });
    ...
    it('remove failure', async function () {
        await viem.assertions.revertWith(iterableOracles.write.removeOracle([ADDRESS_ZERO]), 'Owner address cannot be 0x0');
        await viem.assertions.revertWith(iterableOracles.write.removeOracle([ADDRESS_ONE]), 'Owner not registered');
    });
    it('remove success', async function () {
        ...
        await viem.assertions.revertWith(iterableOracles.write.removeOracle([ownerAddress]), 'Owner not registered');
    });
});
//# sourceMappingURL=IterableOracles.test.js.map