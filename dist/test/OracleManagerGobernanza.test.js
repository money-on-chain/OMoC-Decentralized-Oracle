...
describe('OracleManager by gobernanza', function () {
    ...
    async function setupFixture() {
        ...
        const contracts = await initContracts(deployer, governorOwner, period, minOracleOwnerStake, [whitelistedCaller.account.address]);
        const coinPairPrice = await initCoinpair(deployer, 'BTCUSD', contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address]);
        const oracle = {
            name: 'oracle-a.io',
    ...
    it('Registration and subscription', async function () {
        ...
        await contracts.oracleMgr.write.registerOracle([oracle.owner.account.address, oracle.account.account.address, oracle.name], { account: whitelistedCaller.account });
        ...
        await contracts.oracleMgr.write.subscribeToCoinPair([oracle.owner.account.address, coinPair], { account: whitelistedCaller.account });
    ...
    it('Should fail to unsubscribe oracle if not called by owner', async function () {
        ...
        await viem.assertions.revertWith(contracts.oracleMgr.write.unSubscribeFromCoinPair([accounts[2].account.address, coinPair], { account: accounts[0].account }), 'Address is not whitelisted');
    });
    ...
    it('Should fail to remove oracle if not called by owner', async function () {
        const { viem, contracts, accounts } = await setupFixture();
        await viem.assertions.revertWith(contracts.oracleMgr.write.removeOracle([accounts[2].account.address], {
...
//# sourceMappingURL=OracleManagerGobernanza.test.js.map