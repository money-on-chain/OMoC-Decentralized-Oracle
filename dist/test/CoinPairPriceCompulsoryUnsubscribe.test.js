...
describe('CoinPairPrice - onWithdraw unsubscribes subscribed-but-not-selected oracle', function () {
    ...
    beforeEach(async () => {
        ...
        coinPairPrice = await initCoinpair(deployer, coinPairName, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], maxOraclesPerRound, maxSubscribedOraclesPerRound);
        oracles = [
            {
                name: 'oracle-1.io',
        ...
        for (const data of oracles) {
            ...
            await contracts.staking.write.registerOracle([data.oracle.account.address, data.name], {
                account: data.owner.account,
            });
        ...
        expect(await coinPairPrice.read.isSubscribed([oracles[2].owner.account.address])).to.equal(true);
    });
    ...
    it('oracle3 withdraws all stake and is immediately unsubscribed (fix)', async function () {
        ...
        expect(await contracts.staking.read.getBalance([oracles[2].owner.account.address])).to.equal(0n);
        expect(await coinPairPrice.read.isSubscribed([oracles[2].owner.account.address])).to.equal(false);
    });
...
//# sourceMappingURL=CoinPairPriceCompulsoryUnsubscribe.test.js.map