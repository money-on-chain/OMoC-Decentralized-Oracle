...
import { encodeCoinPair, initCoinpair, initContracts, mineUntilNextRound } from '../src/helpers.js';
...
describe('CoinPairPrice Subscribe', function () {
    ...
    const MIN_SUBSCRIPTION_STAKE = 10000000000n;
    ...
    let viem;
    let accounts;
    async function register(token, staking, oracleMgr, entry) {
    ...
    async function buildScenario() {
        const contracts = await initContracts(deployer, accounts[governorOwnerIndex], 10n, MIN_SUBSCRIPTION_STAKE);
        const coinPairPrice = await initCoinpair(deployer, COINPAIR_NAME, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], MAX_ORACLES_PER_ROUND, MAX_SUBSCRIBED_ORACLES_PER_ROUND);
        ...
        for (let i = 0; i < ORACLE_QUANTITY; i++) {
            ...
            await contracts.governor.mint(contracts.token.address, owner.account.address, 8n * 10n ** 20n);
    ...
    beforeEach(async function () {
        ...
        if (accounts.length < oracleStartIndex + ORACLE_QUANTITY) {
            throw new Error(`Expected at least ${oracleStartIndex + ORACLE_QUANTITY} wallet clients, got ${accounts.length}`);
        }
    });
    describe('During non zero we add oracles on subscription right away', () => {
        ...
        let coinPairPrice;
        let oracleList;
        let oracleWithALotOfStake;
        let oracleWithALessLotOfStake;
        let oracleWithSmallStake;
        ...
        it('Should subscribe oracleCant oracles and the first maxOraclesPerRound goes to current round', async function () {
            ...
            for (const { owner } of oracleList) {
                ...
                expect(await contracts.oracleMgr.read.isSubscribed([owner.account.address, COINPAIR])).to.equal(true);
            ...
            await viem.assertions.revertWith(contracts.staking.write.subscribeToCoinPair([COINPAIR], {
                account: oracleList[0].owner.account,
            }), 'Not enough stake to add');
            expect(await coinPairPrice.read.isOracleInCurrentRound([
                oracleList[0].owner.account.address,
            ])).to.equal(true);
            expect(await coinPairPrice.read.isOracleInCurrentRound([
                oracleWithALotOfStake.owner.account.address,
            ...
            expect(await contracts.staking.read.getBalance([oracleList[idx1].owner.account.address])).to.equal(oracleList[idx1].stake);
            expect(await contracts.staking.read.getBalance([oracleList[idx2].owner.account.address])).to.equal(oracleList[idx2].stake);
            ...
            expect(await contracts.staking.read.getBalance([oracleList[idx1].owner.account.address])).to.equal(0n);
            await contracts.staking.write.withdraw([oracleList[idx2].stake], {
                account: oracleList[idx2].owner.account,
            });
            expect(await contracts.staking.read.getBalance([oracleList[idx2].owner.account.address])).to.equal(0n);
            ...
            expect((await getSelectedOracles()).indexOf(oracleWithSmallStake.oracleAddr) >= 0).to.equal(false);
            expect(await contracts.oracleMgr.read.isSubscribed([
                oracleWithALotOfStake.owner.account.address,
                COINPAIR,
            ])).to.equal(true);
            expect((await getSelectedOracles()).indexOf(oracleWithALessLotOfStake.oracleAddr) >= 0).to.equal(false);
            ...
            expect(await contracts.oracleMgr.read.isSubscribed([owner.account.address, COINPAIR])).to.equal(false);
...
//# sourceMappingURL=CoinPairPriceSubscribe.test.js.map