...
function sortByAddressDesc(entries) {
...
function asOracleDefinition(oracle) {
    return {
        owner: oracle.owner,
...
describe('Staking-withdraw', function () {
    ...
    before(async function () {
        ...
        if (accounts.length < NUM_ORACLES * 2) {
            throw new Error(`Expected at least ${NUM_ORACLES * 2} wallet clients, got ${accounts.length}`);
        }
        contracts = await initContracts(deployer, accounts[governorOwnerIndex], 20n, ORACLE_STAKE);
        coinPairPrice = await initCoinpair(deployer, COINPAIR_NAME, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[governorOwnerIndex].account.address], MAX_SELECTED_ORACLES, MAX_SUBSCRIBED_ORACLES);
        await contracts.governor.mint(contracts.token.address, accounts[feesAccountIndex].account.address, TOKEN_FEES);
        ...
        for (const oracle of oracles) {
            await contracts.governor.mint(contracts.token.address, oracle.owner.account.address, oracle.stake);
        }
    });
    it('subscription', async function () {
        ...
        for (const oracle of oracles) {
            ...
            await contracts.staking.write.deposit([oracle.stake, oracle.owner.account.address], sender);
        ...
        for (let i = 0; i < Number(MAX_SELECTED_ORACLES); i += 1) {
            ...
            expect(await coinPairPrice.read.isOracleInCurrentRound([oracle.owner.account.address])).to.equal(true);
            expect(await coinPairPrice.read.isSubscribed([oracle.owner.account.address])).to.equal(true);
        }
        for (let i = Number(MAX_SELECTED_ORACLES); i < NUM_ORACLES; i += 1) {
            const oracle = oracles[i];
            expect(await coinPairPrice.read.isOracleInCurrentRound([oracle.owner.account.address])).to.equal(false);
            expect(await coinPairPrice.read.isSubscribed([oracle.owner.account.address])).to.equal(true);
        }
    });
    ...
    it('publish price', async function () {
        ...
        const oracle = oracles[Number(MAX_SELECTED_ORACLES) - 1];
        expect(await coinPairPrice.read.isOracleInCurrentRound([oracle.owner.account.address])).to.equal(true);
        const publishingOracles = sortByAddressDesc(Array.from({ length: Number(MAX_SELECTED_ORACLES / 2n + 1n) }, (_, i) => asOracleDefinition(oracles[Number(MAX_SELECTED_ORACLES) - 1 - i])));
    ...
    it('new selected oracle can publish', async function () {
        ...
        expect(await coinPairPrice.read.isOracleInCurrentRound([newOracle.owner.account.address])).to.equal(true);
        const publishingOracles = sortByAddressDesc([
            ...Array.from({ length: Number(MAX_SELECTED_ORACLES / 2n) }, (_, i) => asOracleDefinition(oracles[i])),
...
//# sourceMappingURL=StakingWithdraw.test.js.map