...
import { encodeCoinPair, initCoinpair, initContracts, mineUntilNextRound, publishPrice, toOracleDefinition, } from '../src/helpers.js';
...
describe('Staking-Oracle-States', function () {
    ...
    async function deployContracts() {
        contracts = await initContracts(deployer, accounts[governorOwnerIndex], 20n, minSubscriptionStake);
        coinPairPrice = await initCoinpair(deployer, COINPAIR_NAME, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], maxOraclesPerRound, maxSubscribedOraclesPerRound);
    }
    async function register(data) {
        await contracts.governor.mint(contracts.token.address, data.owner.account.address, data.stake);
        ...
        expect(await contracts.staking.read.getBalance([data.owner.account.address])).to.equal(data.stake);
    ...
    async function checkState(data, state) {
        expect(await contracts.staking.read.isOracleRegistered([data.owner.account.address])).to.equal(state.registered);
        expect(await contracts.staking.read.isSubscribed([data.owner.account.address, coinPair])).to.equal(state.subscribed);
    ...
    beforeEach(async function () {
        ...
        const indices = [...Array(oracleData.length).keys()].sort((a, b) => oracleData[a].stake < oracleData[b].stake
            ? -1
    ...
    describe('Registered Oracles', function () {
        describe('Can unregister', function () {
            it('if they are not subscribed nor selected', async function () {
                ...
                expect(await contracts.staking.read.canRemoveOracle([oracle.owner.account.address])).to.be.true;
        ...
        describe('Can NOT unregister', function () {
            it('if subscribed', async function () {
                ...
                expect(await contracts.staking.read.canRemoveOracle([
            ...
            it('if subscribed and selected', async function () {
                ...
                await viem.assertions.revertWith(contracts.staking.write.removeOracle({ account: oracle.owner.account }), 'Not ready to remove');
            });
    ...
    describe('Selected Oracles', function () {
        ...
        it('Are rewarded at the end of the round when they publish', async function () {
            ...
            await mineUntilNextRound(networkHelpers, viem, coinPairPrice);
            await coinPairPrice.write.switchRound();
            expect(await contracts.token.read.balanceOf([oracle.owner.account.address])).to.equal(rewards);
        });
        it('Are punished when they withdraw the stake in the middle of the round', async function () {
            ...
            await coinPairPrice.write.switchRound();
            expect(await contracts.token.read.balanceOf([oracle.owner.account.address])).to.equal(0n);
        });
...
//# sourceMappingURL=StakingOracleStates.test.js.map