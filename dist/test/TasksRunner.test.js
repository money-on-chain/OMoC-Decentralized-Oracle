...
import { encodeCoinPair, initContracts, mineUntilNextRound, waitForEvents, } from '../src/helpers.js';
...
describe('TasksRunner', function () {
    ...
    let mockTask;
    let revertingTask;
    let mockTokenToCoinbaseProvider;
    let mockBaseFeeProvider;
    async function deployFixture() {
        ...
        await contracts.governor.registerCoinPair(contracts.oracleMgr, TASKS_PAIR, tasksRunner.address);
        await contracts.governor.mint(contracts.token.address, accounts[ORACLE_OWNER].account.address, MIN_STAKE);
        await contracts.token.write.approve([contracts.staking.address, MIN_STAKE], {
            account: accounts[ORACLE_OWNER].account,
        });
        await contracts.staking.write.deposit([MIN_STAKE, accounts[ORACLE_OWNER].account.address], {
            account: accounts[ORACLE_OWNER].account,
        });
        ...
        await contracts.oracleMgr.write.subscribeToCoinPair([accounts[ORACLE_OWNER].account.address, TASKS_PAIR], { account: accounts[WHITELISTED_CALLER].account });
    ...
    it('runs tasks with a single selected oracle even when registry minimum is higher', async function () {
        ...
        const message = buildRunTasksMessage(3n, TASKS_PAIR, tasksFlags, accounts[ORACLE_ACCOUNT].account.address, lastPublicationBlock);
        const signature = parseSignature(await accounts[ORACLE_ACCOUNT].signMessage({
        ...
        assertSameAddress(successEvent.args?.votedOracle, accounts[ORACLE_ACCOUNT].account.address);
        assertSameAddress(revertingEvent.args?.sender, accounts[ORACLE_OWNER].account.address);
        assertSameAddress(revertingEvent.args?.votedOracle, accounts[ORACLE_ACCOUNT].account.address);
...
//# sourceMappingURL=TasksRunner.test.js.map