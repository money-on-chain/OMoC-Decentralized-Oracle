...
describe('StakingRounding', function () {
    ...
    beforeEach(async function () {
        ...
        ALICE = accounts[2];
        BOB = accounts[4];
        contracts = await initContracts(deployer, accounts[8], BigInt(period), BigInt(minCPSubscriptionStake));
    ...
    async function testIt(testValue, token, staking, stakingMock, supporters, delayMachine) {
        ...
        expect(mocBalanceAfterFirstDeposit.toString(), 'mocBalanceAfterFirstDeposit').to.be.equal(testValue.amount);
        // Check the internal token balance was added.
        expect(tokenBalanceAfterFirstDeposit.toString(), 'tokenBalanceAfterFirstDeposit').to.be.equal(testValue.amount);
        ...
        expect((supportersBalanceAfterTransfer - supportersBalanceBeforeTransfer).toString(), 'supportersBalanceAfterTransfer.sub(supportersBalanceBeforeTransfer)').to.be.equal(testValue.reward);
        ...
        if (testValue.addAmount !== '0') {
            ...
            const otherUserTokenBalanceBeforeOtherDeposit = await stakingMock.read.getBalanceInTokens([BOB.account.address]);
            log("ALICE's moc balance before BOB's deposit", mocBalanceBeforeOtherDeposit.toString());
            log("ALICE's token balance before BOB's deposit", tokenBalanceBeforeOtherDeposit.toString());
            log("BOB's moc balance before BOB's deposit", otherUserMocBalanceBeforeOtherDeposit.toString());
            log("BOB's token balance before BOB's deposit", otherUserTokenBalanceBeforeOtherDeposit.toString());
            // Previous approve for deposit in Staking
            await token.write.approve([staking.address, testValue.addAmount], {
                account: BOB.account,
            });
            log("Total mocs before BOB's deposit:", (await staking.read.totalMoc()).toString(), '//', "Total tokens before BOB's deposit:", (await staking.read.totalToken()).toString());
            log("Making BOB's deposit of", testValue.addAmount, 'mocs <---------------');
            // Deposit mocs in Staking
            await staking.write.deposit([testValue.addAmount, BOB.account.address], {
                account: BOB.account,
            });
            log("Total mocs after BOB's deposit:", (await staking.read.totalMoc()).toString(), '//', "Total tokens after BOB's deposit:", (await staking.read.totalToken()).toString());
            const otherUserMocBalanceAfterSecondDeposit = await staking.read.getBalance([
                BOB.account.address,
            ]);
            const otherUserTokenBalanceAfterSecondDeposit = await stakingMock.read.getBalanceInTokens([BOB.account.address]);
            log("BOB's moc balance after deposit:", otherUserMocBalanceAfterSecondDeposit.toString(), 'Expected:', testValue.otherUserMocBalanceAfterSecondDeposit);
            log("BOB's token balance after deposit", otherUserTokenBalanceAfterSecondDeposit.toString(), 'Expected:', testValue.otherUserTokenBalanceAfterSecondDeposit);
            // Check the other user's stake in mocs was deposited
            expect(otherUserMocBalanceAfterSecondDeposit.toString(), 'otherUserMocBalanceAfterSecondDeposit').to.be.equal(testValue.otherUserMocBalanceAfterSecondDeposit);
            // Check the other user's token balance was correctly deposited
            expect(otherUserTokenBalanceAfterSecondDeposit.toString(), 'otherUserTokenBalanceAfterSecondDeposit').to.be.equal(testValue.otherUserTokenBalanceAfterSecondDeposit);
        }
        ...
        const delayMocBalanceChangeAfterWithdrawal = delayMocBalanceAfterWithdrawal - delayMocBalanceBeforeWithdrawal;
        log("Delay Machine's moc balance change after withdrawal:", delayMocBalanceChangeAfterWithdrawal.toString(), 'Expected:', testValue.delayMocBalanceChangeAfterWithdrawal);
        // Assert that delay machine received the amount withdrawn
        expect(delayMocBalanceChangeAfterWithdrawal.toString(), 'delayMocBalanceChangeAfterWithdrawal').to.be.equal(testValue.delayMocBalanceChangeAfterWithdrawal);
        ...
        log("ALICE's moc balance after withdrawal:", mocBalanceAfterWithdrawal.toString(), 'Expected:', testValue.mocBalanceAfterWithdrawal);
        log("ALICE's token balance after withdrawal:", tokenBalanceAfterWithdrawal.toString(), 'Expected:', testValue.tokenBalanceAfterWithdrawal);
        // Check moc balance of user after withdrawal
        expect(mocBalanceAfterWithdrawal.toString(), 'mocBalanceAfterWithdrawal').to.be.equal(testValue.mocBalanceAfterWithdrawal);
        // Check the internal token balance.
        expect(tokenBalanceAfterWithdrawal.toString(), 'tokenBalanceAfterWithdrawal').to.be.equal(testValue.tokenBalanceAfterWithdrawal);
        log('ALICE withdraws the rest of the stake to reset it, which is', mocBalanceAfterWithdrawal.toString(), 'mocs');
        ...
        log("ALICE's moc balance after reset:", mocBalanceAfterReset.toString(), 'Expected:', testValue.mocBalanceAfterReset);
        log("ALICE's token balance after reset:", tokenBalanceAfterReset.toString(), 'Expected:', testValue.tokenBalanceAfterReset);
        // Check the owner's moc balance is 0.
        expect(mocBalanceAfterReset.toString(), 'mocBalanceAfterReset').to.be.equal(testValue.mocBalanceAfterReset);
        // Check the owner's internal token balance is 0.
        expect(tokenBalanceAfterReset.toString(), 'tokenBalanceAfterReset').to.be.equal(testValue.tokenBalanceAfterReset);
        /*
    ...
    describe('Should check several problematic cases where an oracle might loose mocs when making deposits or withdrawals', async () => {
        const testValues = [
            ...
            {
                testNumber: '3',
                description: "alice deposits 1, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can't withdraw 1 MOC",
                amount: '1',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '1',
            ...
            {
                testNumber: '4',
                description: 'alice deposits 1, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can withdraw 2 MOC',
                amount: '1',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '2',
            ...
            {
                testNumber: '5',
                description: 'alice deposits 1, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can withdraw 3 MOC',
                amount: '1',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '3',
            ...
            },
            {
                ...
                description: 'alice deposits 2, has 1 reward, alice tries to withdraw 2, withdraws 1 MOC',
                amount: '2',
                reward: '1',
                addAmount: '0',
                withdrawAmount: '2',
            ...
            {
                testNumber: '9',
                description: "alice deposits 2, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can't withdraw 1 MOC",
                amount: '2',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '1',
            ...
            {
                testNumber: '10',
                description: 'alice deposits 2, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can withdraw 2 MOC and gets 1 MOC',
                amount: '2',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '2',
            ...
            {
                testNumber: '11',
                description: 'alice deposits 2, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice tries to withdraw 3, and gets 3 MOC',
                amount: '2',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '3',
            ...
            {
                testNumber: '12',
                description: 'alice deposits 2, has 1 reward, bob deposits 1 (not enough to buy he gets nothing), alice can withdraw 4 MOC but gets 3',
                amount: '2',
                reward: '1',
                addAmount: '1',
                withdrawAmount: '4',
            ...
            {
                testNumber: '13',
                description: "alice deposits 1, has 1 reward, bob deposits 2, alice can't withdraw 1 MOC",
                amount: '1',
                reward: '1',
                addAmount: '2',
                withdrawAmount: '1',
            ...
            {
                testNumber: '14',
                description: 'alice deposits 1, has 1 reward, bob deposits 2, alice can withdraw 2 MOC',
                amount: '1',
                reward: '1',
                addAmount: '2',
                withdrawAmount: '2',
            ...
            {
                testNumber: '15',
                description: "alice deposits 2, has 1 reward, bob deposits 2 and gets 1, alice can't withdraw 1",
                amount: '2',
                reward: '1',
                addAmount: '2',
                withdrawAmount: '1',
            ...
            {
                testNumber: '16',
                description: 'alice deposits 2, has 1 reward, bob deposits 2 and gets 1, alice tries to withdraw 2, withdraws 1 MOC',
                amount: '2',
                reward: '1',
                addAmount: '2',
                withdrawAmount: '2',
            ...
            {
                testNumber: '17',
                description: 'alice deposits 2, has 1 reward, bob deposits 2 and gets 1, alice tries to withdraw 3, withdraws 1 MOC',
                amount: '2',
                reward: '1',
                addAmount: '2',
                withdrawAmount: '3',
            ...
            {
                testNumber: '18',
                description: "alice deposits 100, has 10000 reward, alice can't withdraw 100 MOC, must withdraw multiples of 101==10100/100",
            ...
            {
                testNumber: '19',
                description: 'alice deposits 100, has 10000 reward, alice withdraw 150 MOC and gets 1 token == 101',
                amount: '100',
        ...
        for (let i = 0; i < testValues.length; i++) {
            it(testValues[i].testNumber + ': ' + testValues[i].description, async () => {
                await testIt(testValues[i], contracts.token, contracts.staking, contracts.stakingMock, contracts.supporters, contracts.delayMachine);
            });
        }
        it('alice deposits 100, has 10000 reward, bob deposit 150 and gets 1 token, alice withdraw 150 MOC and gets 1 token == 101', async () => {
            ...
            await testIt({
                ...
                mocBalanceAfterReset: '0',
                tokenBalanceAfterReset: '0',
            }, token, staking, stakingMock, supporters, delayMachine);
            const bobBalanceAfter = await token.read.balanceOf([BOB.account.address]);
            const bobApprovalAfter = await token.read.allowance([
                BOB.account.address,
                staking.address,
            ]);
            expect(Number(bobBalanceBefore - bobBalanceAfter), "We only take what's needed from bob").to.be.equal(101);
            expect(Number(bobApprovalAfter - bobApprovalBefore), 'We leave the rest untouched').to.be.equal(49);
        });
    });
});
//# sourceMappingURL=StakingRounding.test.js.map