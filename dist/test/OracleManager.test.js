...
import { bytesToHex, getAddress, parseEther, stringToHex } from 'viem';
import { initContractsWithCoinPairs, ADDRESS_ZERO } from '../src/helpers.js';
...
describe('OracleManager', function () {
    ...
    before(async () => {
        ...
        contracts = await initContractsWithCoinPairs(deployer, governorOwner, undefined, minOracleStake, [accounts[0].account.address, whitelisted.address]);
        oracleMgr = contracts.oracleMgr;
        for (const i of [0, 2, 4, 6]) {
            await contracts.governor.mint(contracts.token.address, accounts[i].account.address, parseEther('800'));
        }
        oracleData = [
            {
                name: 'oracle-a.io',
    ...
    it("Should fail to register Oracle from Oracle Manager calling from an address other than the Staking contract's", async () => {
        ...
        await viem.assertions.revertWith(oracleMgr.write.registerOracle([o.owner.account.address, o.address, o.name], {
            account: o.owner.account,
        }), 'Address is not whitelisted');
    });
    it('Should fail to register Oracle with null address as oracle address', async () => {
        await viem.assertions.revertWith(oracleMgr.write.registerOracle([ADDRESS_ZERO, ADDRESS_ZERO, 'mock.io'], caller), 'Owner address cannot be 0x0');
    });
    it('Should fail to register an Oracle twice', async () => {
        const o = oracleData[0];
        await viem.assertions.revertWith(oracleMgr.write.registerOracle([o.owner.account.address, o.address, o.name], caller), 'Owner already registered');
    });
    ...
    it('Should fail to subscribe oracle if not called by owner', async () => {
        await viem.assertions.revertWith(oracleMgr.write.subscribeToCoinPair([oracleData[0].account.account.address, coinPair], caller), 'Oracle not registered');
    });
    it('Should fail to unsubscribe oracle if not called by owner', async () => {
        await viem.assertions.revertWith(oracleMgr.write.unSubscribeFromCoinPair([oracleData[0].account.account.address, coinPair], caller), 'Oracle not registered');
    });
    it('Should fail to subscribe oracle if already subscribed', async () => {
        await viem.assertions.revertWith(oracleMgr.write.subscribeToCoinPair([oracleData[0].owner.account.address, coinPair], caller), 'Oracle is already subscribed to this coin pair');
    });
    ...
    it('Should fail to unsubscribe oracle if not subscribed', async () => {
        await viem.assertions.revertWith(oracleMgr.write.unSubscribeFromCoinPair([oracleData[0].owner.account.address, coinPair], caller), 'Oracle is not subscribed to this coin pair');
    });
    it('Should reject to change name of unregistered oracle', async () => {
        ...
        await viem.assertions.revertWith(oracleMgr.write.setOracleName([randomAddr, 'X'], caller), 'Oracle not registered');
    });
    it('Should reject to change name of oracle if called by non-owner', async () => {
        await viem.assertions.revertWith(oracleMgr.write.setOracleName([oracleData[0].address, 'XYZ'], caller), 'Oracle not registered');
    });
    ...
    it('Should fail to remove an oracle if is not registered', async () => {
        await viem.assertions.revertWith(oracleMgr.write.removeOracle([oracleData[3].owner.account.address], caller), 'Oracle not registered');
    });
    ...
    it('Should fail to remove an oracle if it is in a current round', async () => {
        ...
        await viem.assertions.revertWith(oracleMgr.write.removeOracle([address], caller), 'Not ready to remove');
    });
...
//# sourceMappingURL=OracleManager.test.js.map