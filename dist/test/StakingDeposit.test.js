...
describe('Staking-deposit', function () {
    ...
    describe('Problem: Deposit in third party accounts is a security issue, fund in vesting-machine can be withdrawn', function () {
        ...
        it('Deposits to third parties is forbidden', async function () {
            ...
            await viem.assertions.revertWith(contracts.staking.write.deposit([amount, accounts[governorOwnerIndex].account.address], {
                account: user.account,
            }), 'FIX: Only sender');
        });
    });
    describe('Problem: when exposed depositFrom is too risky, a user can take someones approved funds and deposit them in their own account', function () {
        it('The delay machine can call depositFrom', async function () {
            ...
            expect(await contracts.token.read.balanceOf([contracts.delayMachine.address])).to.equal(amount);
            ...
            expect(await contracts.staking.read.getBalance([user.account.address])).to.equal(amount);
            expect(await contracts.token.read.balanceOf([contracts.delayMachine.address])).to.equal(0n);
        });
        it('Should fail to call depositFrom if the caller is not the delay machine', async function () {
            ...
            await viem.assertions.revertWith(contracts.staking.write.depositFrom([amount, user.account.address, user.account.address], {
                account: user.account,
            }), 'delayMachineOnly');
        });
    });
});
//# sourceMappingURL=StakingDeposit.test.js.map