...
describe('CoinPairPrice Emergency Publish', function () {
    ...
    async function buildFixture() {
        ...
        const coinPairPrice = await initCoinpair(deployer, coinPairName, contracts.governor, contracts.token, contracts.oracleMgr, contracts.registry, [accounts[0].account.address], 10n, 10n, 60n, 0n, 30n, emergencyPublishingPeriodInBlocks);
    ...
    it('Should fail to emergency publish if not whitelisted', async function () {
        ...
        await viem.assertions.revertWith(coinPairPrice.write.emergencyPublish([1234n], {
            account: accounts[1].account,
        }), 'Address is not whitelisted');
    });
    it('Should fail to publish a zero price', async function () {
        const { coinPairPrice } = await buildFixture();
        await viem.assertions.revertWith(coinPairPrice.write.emergencyPublish([0n], {
    ...
    it('Should success to emergency publish after a regular publication and emergencyPublishingPeriodInBlocks blocks', async function () {
        ...
        for (const oracle of oracles) {
            await contracts.governor.mint(contracts.token.address, oracle.owner.account.address, minSubscriptionStake);
            await contracts.token.write.approve([contracts.staking.address, minSubscriptionStake], {
                account: oracle.owner.account,
            });
            await contracts.staking.write.deposit([minSubscriptionStake, oracle.owner.account.address], {
                account: oracle.owner.account,
            });
...
//# sourceMappingURL=CoinPairPriceEmergencyPublish.test.js.map