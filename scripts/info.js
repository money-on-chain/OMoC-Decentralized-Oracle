...
async function principal(conf) {
    if (true) {
        ...
        for (let i = 0; i < coinCant; i++) {
            ...
            await helpers.printProps(
                'PROPS ' + helpers.coinPairStr(coinPair) + ' -> ' + addr,
                conf.coinPairPrice,
            );
        }
    ...
    if (true) {
        ...
        await supportersInfo(
            conf.oracleManager.contract,
            conf.oracleManager.addr,
            conf.supporters.contract,
        );
    }
}
