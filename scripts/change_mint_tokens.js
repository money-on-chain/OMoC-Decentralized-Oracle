...
async function main() {
    ...
    const testMOC = await helpers.ozGetContract(
        artifacts,
        '@money-on-chain/omoc-sc-shared/TestMOC',
        '@money-on-chain/omoc-sc-shared/IMintableERC20',
    );
    console.log('using TestMOC: ', testMOC.address);
    const governor = await helpers.ozGetContract(artifacts, '@money-on-chain/omoc-sc-shared/Governor');
