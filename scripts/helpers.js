...
function getScriptArgs(filename) {
    ...
    const scriptInArgs = process.argv.map((x) => x.indexOf(scriptName) > 0);
    const idx = scriptInArgs.indexOf(true);
    if (idx < 0) {
        console.error('INVALID ARGS', scriptInArgs);
        process.exit();
    }
